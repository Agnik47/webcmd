...
webcmd browser work run --stdin --timeout 45 --snapshot-diff
```
...
seconds, `--max-output` bounds returned results and logs, `--snapshot-diff`
returns a structural before/after diff for that run, and `--tab <page-id>`
targets an existing tab.

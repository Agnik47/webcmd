...
gateway value in its isolated launch environment. Stop it later with `Ctrl-C`.
...
7. One checkout call moves the attempt to `pending_payment` and returns a
   short-lived WebCMD-owned hosted browser link. The raw District checkout URL
   stays inside the hosted command. Complete login and payment only in that
   browser handoff.
...
- The short-lived WebCMD viewer link may remain in the user's Hermes
  transcript, although it is excluded from booking-history records.

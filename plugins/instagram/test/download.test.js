...
vi.mock('@agentrhq/webcmd/download', async () => {
    const actual = await vi.importActual('@agentrhq/webcmd/download');
...
let cmd;
const tempDirs = [];
const repoInstagramTestDir = path.resolve('instagram-test');
...
afterEach(() => {
    ...
    fs.rmSync(repoInstagramTestDir, { recursive: true, force: true });
});

function makeTempDownloadRoot() {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'webcmd-instagram-download-'));
    tempDirs.push(dir);
    return dir;
}
...
describe('instagram download command', () => {
    ...
    it('downloads media and prints saved directory', async () => {
        ...
        const downloadRoot = makeTempDownloadRoot();
        const savedDir = path.join(downloadRoot, 'DWUR_azCWbN');
        ...
        expect(mockHttpDownload).toHaveBeenNthCalledWith(1, 'https://cdn.example.com/photo.webp?foo=1', path.join(savedDir, 'DWUR_azCWbN_01.webp'), expect.objectContaining({ timeout: 60000 }));
        expect(mockHttpDownload).toHaveBeenNthCalledWith(2, 'https://cdn.example.com/video.mp4?bar=2', path.join(savedDir, 'DWUR_azCWbN_02.mp4'), expect.objectContaining({ timeout: 120000 }));
        expect(logSpy).toHaveBeenCalledWith(`📁 saved: ${savedDir}`);
        expect(fs.existsSync(path.resolve('instagram-test'))).toBe(false);
    });
